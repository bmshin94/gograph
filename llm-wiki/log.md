...
## [2026-08-25] feature | Add explicit Go build-tag contexts
...
- 2026-08-25: Prepared v1.6.6 for issue #36. Repository analysis now authorizes `go.work` sibling modules only within the nearest real Git checkout, allows unrelated regular/dangling non-Go file links, keeps workspace manifests and machine-validation `--repo` boundaries strict, and bumps source-policy identity to v2. Added strict precise-build exit semantics, repository-aware doctor diagnostics, safe stale wiki-package cleanup, workspace manifest help, and matching CLI/MCP capabilities/docs. Full tests, race detector, lint, staticcheck, vulnerability scans, Hugo build, strict precise self-build, and uncommitted graph review passed before publication.
- 2026-08-25: Published gograph v1.6.6 from release commit `d812a814d436184cd0e7310915063bbd71e3b7e5`. Release workflow `32890959351`, main CI, and documentation deployment passed; all 14 GitHub assets, Homebrew cask 1.6.6 hashes/URLs, and active official MCP Registry version 1.6.6 were independently verified matching.
...
CLI `routes` and MCP `gograph_routes` now share deterministic `gograph.routes.v1` pages with production-only defaults, term/module filters, explicit test inclusion, 100/200 row limits, a 64 KiB compact-result budget, and opaque continuation cursors. The CLI JSON envelope also mirrors the MCP page metadata (`total`, `returned`, `truncated`, and `next_cursor`), and root modules accept the repository-directory basename as a selector. Variadic Gin/Fiber registrations use the final argument as handler, Echo retains its handler-before-middleware order, and MCP capabilities expose the running binary version. Pagination was chosen over whole-result refusal so large multi-module censuses remain completely enumerable without spill files or silent truncation.
...
## [2026-08-27] maintenance | Publish v1.6.8 bounded route census release

- Released gograph v1.6.8 from tag commit `a3facfdd34875762798a13657d069445f3474541`, incorporating bounded paginated CLI/MCP route censuses, production-only defaults with explicit test opt-in, term/module selectors, corrected Gin/Fiber handler selection, root-module basename aliases, CLI JSON pagination metadata, and MCP version reporting.
...
## [2026-09-11] maintenance | v1.7.2 excluded skill symlink repair and publication

Recorded the build-selection security decision: explicitly excluded directory symlinks are hidden from Go production/test loading through a temporary deletion overlay; targets are not traversed, linked build inputs/metadata remain rejected, and operator overlays are not silently replaced. Regressions cover the reported relative skills layout, external targets, direct/test/transitive/replacement imports, precision-preserving MCP refresh, CLI/MCP row parity, and cleanup. Added Codex setup instructions verified against official documentation and local CLI help. Published immutable v1.7.2 at dc63800 (fix 6eeb07b); release 34589507999, source CI 34589507775, and docs 34589507796 passed. All 14 GitHub assets and active Registry hashes match. Homebrew tap 2378496 installed 1.7.2; the installed binary passed the retained-symlink strict-build reproduction and fresh 68-tool MCP acceptance, with identical four-row query results and unchanged graph bytes. Existing MCP processes require restart. See build-selection.md and mcp-registry.md for boundaries and evidence.


## 2026-09-17 — THE-READ-THAT-ANSWERED-NOTHING

Measured v1.7.2 CLI and stdio MCP on one scratch fixture: source/context text contained the answer, while their structured companion contained only schema_version and graph_state. This was a Gograph presentation defect, including source errors whose reasons existed only in text. Red-first fixture checks failed for both existing and absent symbols; the new gograph.read.v1 response carries source/context evidence or the named refusal. Missing contexts now refuse; ambiguity lists candidates; oversized source reports its size and 65536-byte budget. A read_diagnostic records source unavailability independently of analysis precision/freshness. Source confinement and verdict/audit rules remain in force. Restart MCP servers after upgrading. The consumer's gograph-first hook is harness-owned at wiki/tools/gograph-first-hook.sh and remains outside this change. Release and installation outcomes are measured separately, not inferred from these fixture tests.
