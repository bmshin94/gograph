...
func NewServer(
	...
	addTool(capabilitiesTool, func(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		resp := map[string]any{
			...
			"sql_static_classification": map[string]any{
				...
				"extraction":         "direct literals plus statically resolvable local or same-file package const/var declarations, straight-line assignments, and bounded string concatenations; runtime-generated SQL remains excluded",
			...
			"tools": []map[string]string{
				...
				{"name": "gograph_implementers", "purpose": "Structs that implement a named interface. Type-checked production results merge AST-discovered test-file fakes; set test_only=true for mocks/stubs only."},
				...
				{"name": "gograph_boundaries", "purpose": "Verify analyzed imports against a regular, non-linked policy (default .gograph/boundaries.json; override with config). MCP refreshes before evaluation; CLI reads the persisted graph."},
				...
				{"name": "gograph_routes", "purpose": "Bounded HTTP route pages with term/module filters, production-only default, optional tests, and deterministic next_cursor pagination. CLI --files-only is a complete file census, not a route-row dump. Use before gograph_endpoint."},
				...
				{"name": "gograph_tests", "purpose": "Direct attributed test calls by default; Receiver.Method and stable-ID selectors are supported. transitive=true returns gograph.tests.v1 reverse attribution with every reaching test, exact/possible propagation, depth, and stable-ID paths."},
				...
				{"name": "gograph_sql", "purpose": "Bounded PostgreSQL static SQL pages from direct or statically resolved declared strings, with table, verb, access, function, module, test, and cursor filters plus explicit classification."},
				...
				{"name": "gograph_usages", "purpose": "Type references in signatures, fields, interface methods, and composite-literal construction. Use gograph_literals for only Foo{...} sites."},
	...
	routesTool := mcp.NewTool("gograph_routes",
		...
		mcp.WithString("module", mcp.Description("Optional exact module path/directory, unique nested-module directory basename, or repository directory name for a root module (for example identuum-idp)")),
